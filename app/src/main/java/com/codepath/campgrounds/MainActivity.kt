...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var campgroundAdapter: CampgroundAdapter // Added for Step 3
            ...
            (application as CampgroundApplication).db.campgroundDao().getAll().collect { databaseList ->
                databaseList.map { entity ->
                    Campground(
                        ...
                        entity.description,
                        entity.latLong,
                        if (entity.imageUrl != null) listOf(CampgroundImage(entity.imageUrl, null)) else null
                    )
                }.also { mappedList ->
                    campgrounds.clear()
                ...
                try {
                    ...
                    parsedJson.data?.let { list ->
                        lifecycleScope.launch(IO) {
                            ...
                            (application as CampgroundApplication).db.campgroundDao().insertAll(list.map { campground ->
                                CampgroundEntity(
